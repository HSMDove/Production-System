    ...
    try {
      ...
      (async () => {
        try {
          ...
          const fallbackName = (event as any).username || platformUser!.name || undefined;
          ...
          const [slackDisplayName, conversation, result] = await Promise.all([
            namePromise,
          ...
          if (slackDisplayName) console.log(`[Slack] Sender name: "${slackDisplayName}"`);
